...
% Setup the vectors, 'a', 'b', 'c', and 'r' for tridiagonal matrix solution
    ...
    %
        ...
        if rhc == 1     % if root hydraulic conductivity is allowed to increase
            for i = 1:nl_soil
                rootr(i) = max(0, rootfr(i)*((volliq(i)-thetadry(i))/(eff_porosity(i)-thetadry(i)))); 
                % effect of soil moisture include the effect of root conductivity. 
                % Here, a linearly increasing function is considered
